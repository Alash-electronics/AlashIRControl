/*
 * Бұл бағдарлама ИҚ-қабылдағыштармен жұмыс істеу үшін AlashIRControl кітапханасын пайдаланады.
 * Пультте батырма басылып тұрғанда, батырма коды мен хабарлама ұзындығы сериялық мониторда көрсетіледі.
 * Код сигналдарды дұрыс танып-білу үшін деректерді жіберу хаттамасын орнатуды да қамтиды.
*/
...
void setup() {
  ...
  irReceiver.protocol("Ae`@gxYL@L`|LK`I@@@@@BPCp"); // Жауап беру үшін деректерді жіберу хаттамасын орнату
}
