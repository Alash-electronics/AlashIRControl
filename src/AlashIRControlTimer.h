...
//
//	Timer configuration for the AlashIRControl library
//	Developed by Айтен Бексултан from Alash-electronics.kz
//
//  Licensed under the MIT License
//  Copyright (c) 2024 Alash Electronics
//	For support: https://github.com/Alash-electronics/AlashIRControl
//
...
void	Timer_Begin(uint32_t freq){																//	Параметр: частота прерываний в Гц.
		...
		#elif defined(ESP32)																//
		//	ESP32 Arduino Core 3.x использует новый API, 2.x использует старый API			//
			#if ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(3, 0, 0)					//
		//	Новый API для ESP32 Arduino Core 3.x+:											//
			//	Остановка таймера (если был инициализирован):								//
				if( Esp32Timer != NULL ){													//
					timerEnd(Esp32Timer);													//	Останавливаем и удаляем таймер.
					Esp32Timer = NULL;														//
				}																			//
			//	Инициализация и запуск таймера:												//
				Esp32Timer = timerBegin(freq);												//	Создаём таймер с указанной частотой в Гц.
				timerAttachInterrupt(Esp32Timer, &timer_callback_ESP32);					//	Подключаем функцию обработки прерываний.
				timerAlarm(Esp32Timer, 1, true, 0);											//	Устанавливаем счетчик на 1, автоперезагрузка, без ограничения.
			#else																			//
		//	Старый API для ESP32 Arduino Core 2.x:											//
			...
			//	Инициализация таймера (если еще не инициализирован):						//
				if( Esp32Timer == NULL ){													//
					Esp32Timer = timerBegin(2, prescaler, true);							//	Создаём таймер 2 с предделителем.
				}																			//
			//	Остановка таймера (если был запущен):										//
			...
			//	Установка регистров 2 таймера и его запуск на прерывания:					//
				...
				timerStart(Esp32Timer);														//	Запускаем таймер.
			#endif																			//
			#elif defined(ESP8266)																//
			...
			#else																				//
				#error Библиотека AlashIRControl не поддерживает ваш микроконтроллер.			//
			#endif																				//
}																								//
